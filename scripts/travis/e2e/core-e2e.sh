...
node ./scripts/check-env/check-ps-env.js --host "$E2E_HOST_PREVIOUS" -u "$E2E_USERNAME" -p "$E2E_PASSWORD" || exit 1
node ./scripts/check-env/check-cs-env.js --host "$E2E_HOST_PREVIOUS" -u "$E2E_USERNAME" -p "$E2E_PASSWORD" || exit 1
RUN_E2E=$(echo ./scripts/test-e2e-lib.sh -host http://localhost:4200 -proxy "$E2E_HOST_PREVIOUS" -u "$E2E_USERNAME" -p "$E2E_PASSWORD" -e "$E2E_EMAIL"  --use-dist -b -m 3  )
